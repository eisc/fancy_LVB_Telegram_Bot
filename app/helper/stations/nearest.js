exports.getNearestStationsForSelection = function (allStops, location, maxNearest) {
  const dataWithDistance = allStops.map(stop => {
    stop.distance = Math.sqrt(Math.pow((location.longitude - stop.lon) * Math.cos((location.latitude + stop.lat) / 2), 2) + Math.pow(location.latitude - stop.lat, 2)) * 63710
    return stop
  ...
  return selection.map(station => {
    return [{ text: station.name + ` (${Math.round(station.distance)}m)`,
    callback_data: JSON.stringify({
      stationFromLocationSelection: station.id
    }, null, 2)}]
  })
}
